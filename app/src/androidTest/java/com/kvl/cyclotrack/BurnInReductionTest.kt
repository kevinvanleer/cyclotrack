...
class BurnInReductionTest() {
    ...
    fun init() {
        ...
        PreferenceManager.getDefaultSharedPreferences(getInstrumentation().targetContext.applicationContext)
            .edit {
                ...
                putBoolean(
                    context.getString(
                        R.string.preference_key_burn_in_reduction
                    ), true
                )
                putBoolean(
                    context.getString(
        ...
        try {
            ...
            Log.d("BurnInReductionTest", "Mock location provider was not removed last test")
        ...
        try {
            ...
            Log.d("BurnInReductionTest", "Mock location provider already added")
        }
        tripsDao.wipe()
    }
    ...
    fun test_burnInReduction() {
        ...
        testData.sortedBy { it.time }.forEach {
            if (it.isComplete) {
                CadenceData(
                    50,
                    0,
                    0, 80f + (Math.random() * 10).toFloat(), now
                ).let { c ->
                    EventBus.getDefault().post(c)
                }
                HrmData(
                    50,
                    (155 + Math.random() * 10).toInt().toShort(),
                    0, "", now
                ).let { c ->
                    EventBus.getDefault().post(c)
                }
