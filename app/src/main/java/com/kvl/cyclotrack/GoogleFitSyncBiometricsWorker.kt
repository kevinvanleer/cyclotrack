...
class GoogleFitSyncBiometricsWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        if (useGoogleFitBiometrics(applicationContext)) {
            ...
            sharedPreferences.edit {
                height?.let {
                    putString(
                        applicationContext.getString(R.string.preference_key_biometrics_user_height),
                        String.format(
                            "%.1f",
                ...
                weight?.let {
                    putString(
                        applicationContext.getString(R.string.preference_key_biometrics_user_weight),
                        String.format("%.1f", convertSystemToUserMass(it, applicationContext))
                    )
                }

                restingHr?.takeIf { useGoogleFitRestingHeartRate(applicationContext) }?.let {
                    putString(
                        applicationContext.getString(R.string.preference_key_biometrics_user_restingHeartRate),
                        it.toString()
                    )
                }
                commit()
